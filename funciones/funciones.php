<?php
	...
	function getProdImage($producto) {
	  ...
	  //OBTIENE UNA IMAGEN DEL PRODUCTO//
	  function getProdImagesecundaria($producto) {
		$connection = conn();
		$sql = "SELECT * FROM tb_producto_img WHERE id_producto = '$producto' AND tb_producto_img.orden = 2 ORDER BY orden asc LIMIT 1";
		$query = $connection->prepare($sql);
		$query->execute();
...
function enviarPagopar($idPedido, $total_envio, $total_compra, $id_comprador, $ruc, $email, $nombre, $apellido, $telefono, $direccion, $cedula,
	...
	function getProductosBloque($tienda, $bloque, $limit) {
		$connection = conn();
		$sql ="SELECT tb_producto.id as id, 
		tb_producto.nombre as nombre, 
		tb_producto.precio as precio		
		FROM tb_producto_categoria, tb_producto, bloques_productos 
		WHERE tb_producto_categoria.id_producto = tb_producto.id 
		AND tb_producto_categoria.id_categoria = '$tienda'
		AND bloques_productos.id_bloque = '$bloque'
		AND tb_producto.activo = '1' AND tb_producto.id = bloques_productos.id_producto ORDER BY tb_producto.oden_destaque ASC limit ".$limit;
		$query = $connection->prepare($sql);
		$query->execute();
