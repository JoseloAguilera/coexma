<?php 
	...
	function newProducto ($referencia, $modelo, $nombre, $descripcion, $detallada, $precio, $descuento, $categorias, $cod_marca, $atributos, $destacado, $activo, $linea) {
		...
		try {
			...
			$sql = "INSERT INTO tb_producto (referencia, modelo, nombre, descripcion_corta, descripcion_detallada, precio, valor_descuento, id_marca, activo, destaque, id_linea)

		 			VALUES ('$referencia','$modelo', '$nombre', '$descripcion', '$detallada', $precio, $descuento, $cod_marca, $activo, $destacado, $linea)";
	...
	function saveProducto ($codigo, $modelo, $referencia, $nombre, $descripcion, $detallada, $precio, $descuento, $categorias, $cod_marca, $atributos, $destacado, $activo, $linea) {
		...
		try {
			...
			if ($query->rowCount() > 0) {
				...
				$sql = "UPDATE tb_producto SET referencia = '$referencia', modelo = '$modelo',  nombre = '$nombre', descripcion_corta = '$descripcion', descripcion_detallada = '$detallada', precio = '$precio', valor_descuento = '$descuento', id_marca = $cod_marca, activo = $activo, destaque = $destacado, id_linea = $linea
	...
	function updateAllStock () {
		...
		try {
			//eliminamos todo de producto Stock

			$sql = "DELETE FROM tb_producto_stock";
			$query = $connection->prepare($sql);
			$query->execute();

			//seleccionamos todos los productos cuyo precio sea mayor a 1000

			$sql = "SELECT * from tb_producto WHERE precio > 1000";
			$query = $connection->prepare($sql);
			$query->execute();
			...
			if ($query->rowCount() > 0) {
				...
				foreach ($productos as $producto) {
					...
					$sql = "INSERT INTO tb_producto_stock (id_producto, precio, stock)
		 			VALUES ('$idprod', '$precio', 10)";
